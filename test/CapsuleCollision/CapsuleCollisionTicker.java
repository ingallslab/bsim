...
public class CapsuleCollisionTicker extends BSimTicker {
    ...
    final ArrayList<CapsuleCollisionBacterium> bac_bornA;
    final ArrayList<CapsuleCollisionBacterium> bac_bornB;
    final ArrayList<CapsuleCollisionBacterium> bac_deadA;
    final ArrayList<CapsuleCollisionBacterium> bac_deadB;

    // internal machinery - don't worry about this
    final Mover mover;

    public CapsuleCollisionTicker(BSim sim, ArrayList<CapsuleCollisionBacterium> bacA, ArrayList<CapsuleCollisionBacterium> bacB,
    ...
    public void growBacteria( ArrayList<CapsuleCollisionBacterium> bac, ArrayList<CapsuleCollisionBacterium> bacBorn ) {
    	Random bacRng = new Random(); 			// Random number generator
    	bacRng.setSeed(50); 					// Initializes random number generator
    	
        for (CapsuleCollisionBacterium b : bac) { 				// Loop over bac array
            b.grow();

            // Divide if grown past threshold
            if (b.L >= b.L_th) {
            	CapsuleCollisionBacterium daughter = b.divide();
        ...
        for ( CapsuleCollisionBacterium b : bacBorn ) {
    ...
    public void removeBacteria( BSim sim, ArrayList<CapsuleCollisionBacterium> bac, ArrayList<CapsuleCollisionBacterium> bac_dead ) {

        for (CapsuleCollisionBacterium b : bac) {
